...
All endpoints live under `/api/v1`. Examples use `https://api.example.com` as a placeholder, so replace it with your production or staging base URL.
...
You get an overall `status` plus supporting signals (for example `is_disposable`, `is_role_based`, `is_catchall`, and DNS/SMTP checks). Treat `exists` as deliverable, `not_exists` as a hard fail, `invalid_syntax` as invalid input, and `unknown` or `catchall` as cases that need your business logic.
